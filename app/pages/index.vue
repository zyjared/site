...
<template>
  <div>
    <!-- 主内容网格布局 -->
    <div class="grid grid-cols-12 mx-auto min-h-[calc(100vh-4rem)] gap-8 px-4 container" md="gap-4 px-2">
      <!-- layout: 左侧装饰 -->
      <div class="col-span-1 hidden flex-col-center gap-3" md="flex">
        <div class="stagger-fade h-32 w-[1px] from-transparent via-shared/20 to-transparent bg-gradient-to-b" />
        <div
          class="origin-center whitespace-nowrap text-xs text-shared/40 tracking-0.5em uppercase write-vertical-right space-y-3"
        >
      ...
      <!-- layout: 主内容 -->
      <div class="col-span-12 min-h-full flex-col-center" md="col-span-10">
        ...
        <div class="mb-6 flex-col-center gap-6 opacity-40" md="gap-8 mb-8">
          ...
          <!-- 分隔 -->
          <div class="h-[1px] w-48 from-transparent via-shared/20 to-transparent bg-gradient-to-r" />
      ...
      <!-- layout: 右侧社交 -->
      <div
        class="fixed bottom-20 right-4 flex-col-center gap-4"
        md="static col-span-1 gap-6"
      >
        <DarkMode class="stagger-fade mb-2 text-xl" un-md="mb-20" />
        <NuxtLink
          v-for="(social, idx) in socials" :key="idx" :to="social.link"
          external
          class="group stagger-fade flex-center rounded-full bg-shared/5 p-2 text-shared/60"
          un-hover="scale-110 transition-colors hover:bg-shared/20 theme-text"
        >
          <i :class="social.icon" class="block text-xl" />
        </NuxtLink>
      </div>
    </div>
